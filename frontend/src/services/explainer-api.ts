...
export async function fetchVisionHealth(): Promise<{
  ...
  const res = await apiFetch(`${VISION}/health`);
...
export async function uploadExplainerImage(file: File): Promise<ExplainerPage> {
  ...
  fd.append("file", file);
  const res = await apiFetch(`${VISION}/upload`, { method: "POST", body: fd });
...
export async function analyzeExplainerPage(
  ...
  const res = await apiFetch(`${VISION}/analyze`, {
    method: "POST",
...
export async function fetchExplainerPage(pageId: string): Promise<ExplainerPage> {
  const res = await apiFetch(`${VISION}/page/${pageId}`);
  if (!res.ok) throw new Error(await res.text());
  const data = await res.json();
...
export async function confirmExplainerDrill(
  ...
  const res = await apiFetch(`${VISION}/confirm-drill`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function cancelExplainerDrill(pageId: string): Promise<void> {
  const res = await apiFetch(`${VISION}/confirm-drill/${pageId}`, { method: "DELETE" });
  if (!res.ok) throw new Error(await res.text());
}

export async function generateFromTopic(topic: string) {
  const res = await apiFetch(`${GENERATE}/generate-from-text`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function getKnowledgeBaseStatus(kbId: string): Promise<KbEntry> {
  const res = await apiFetch(`${KB}/status/${kbId}`);
  if (res.status === 404) {
...
export async function listKnowledgeBases(): Promise<KbEntry[]> {
  const res = await apiFetch(`${KB}/list`);
  if (!res.ok) await throwIfResponseNotOk(res);
  return res.json();
}

export async function deleteKnowledgeBase(kbId: string): Promise<void> {
  const res = await apiFetch(`${KB}/${kbId}`, { method: "DELETE" });
  if (!res.ok) await throwIfResponseNotOk(res);
}
...
export async function streamExplainerPage(
  ...
  const res = await apiFetch(`${VISION}/stream-page`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      ...params,
