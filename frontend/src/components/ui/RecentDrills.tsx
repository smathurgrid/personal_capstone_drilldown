        ...
        {drills.map((drill) => (
          ...
          >
            ...
            <div className="p-6 flex flex-col flex-grow">
              ...
              {drill.layerCount && drill.layerCount > 1 && (
                <div className="mt-5 pt-4 border-t border-white/5 flex items-center justify-between gap-4">
                  <span className="font-mono text-[10px] uppercase tracking-widest text-secondary font-bold shrink-0">
                    {drill.layerCount} Layers
                  </span>
                  {drill.stages && drill.stages.length > 0 && (
                    <div className="flex -space-x-2 overflow-hidden">
                      {drill.stages.slice(-4).map((stage) => (
                        <div
                          key={stage.id}
                          className="w-9 h-9 rounded-md border border-white/15 bg-black overflow-hidden shadow-lg"
                          title={stage.label}
                        >
                          <img
                            src={stage.imageUrl}
