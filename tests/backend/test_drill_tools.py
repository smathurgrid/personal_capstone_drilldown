...
def test_build_flux_prompt_modes():
    from backend.shared.drill_mode import build_flux_prompt

    inside = build_flux_prompt("turbine blade interior", "inside")
    assert "macro illustration" in inside
    assert "turbine blade" in inside

    pov = build_flux_prompt("harbor view from window", "pov")
    assert "First-person" in pov
    assert "harbor view" in pov
    assert "red circles" in pov


def test_drill_analyzer_pov_skips_parent_paragraph():
    from backend.shared.config import settings
    from backend.services.explainer.drill_analyzer import DrillAnalyzer
    ...
    prompt = analyzer._build_prompt(
        drill_mode="pov",
        pov_context_kind="segment",
        parent_context={
            "metadata": {
                "editorial_headline": "Engine Block",
                "explainer_paragraph": "Internal cross-section of piston assembly.",
            }
        },
    )
    assert "Parent layer: Engine Block" in prompt
    assert "Internal cross-section" not in prompt
