...
def _spread_positions(n: int) -> list[tuple[float, float]]:
    """Deterministic, non-overlapping marker positions (normalized 0-1) for N
    predicted hotspots, used when we skip the per-label locate VLM call. Spreads
    points evenly across the width with a slight vertical zig-zag so markers never
    ...
    if n == 1:
        return [(0.5, 0.5)]
    return [
        (round((i + 1) / (n + 1), 4), 0.42 if i % 2 == 0 else 0.58)
        for i in range(n)
    ]
...
def _free_workers(branches: dict[str, HotspotBranch], keep: HotspotBranch | None = None) -> None:
    """Cancel hotspots that are STILL generating so the next layer's jobs aren't
    queued behind abandoned ones — finished hotspots keep their cached image for Back."""
...
async def _click_point_overlapped(
    ...
    try:
        ...
        # Free the workers for this child's hotspots — cancel the previous layer's
        # still-generating ones (finished ones stay cached for Back).
        _free_workers(sess.branches)
        sess.archive.extend(sess.branches.values())
        sess.branches = {}
        sess.depth += 1
        sess.parent_b64 = child_b64
        ...
        # 5. Place ALL predicted hotspots at deterministic spread positions and
        #    skip the per-label locate VLM call. This removes a full vision
        #    round-trip from the critical path, so markers appear the instant the
        #    child renders instead of after "EXTRACTING LABELS…". Trade-off:
        #    positions are approximate (we trust the prediction, not a re-find of
        #    each label in the generated image), so a marker may not sit exactly on
        #    its component.
        kept = list(pred_branches)
        for b, (sx, sy) in zip(kept, _spread_positions(len(kept))):
            b.x, b.y = sx, sy
...
async def go_back(session_id: str) -> AsyncIterator[str]:
    ...
    try:
        ...
        # Restore this layer's FINISHED hotspots from cache (instant). Ones that were
        # still generating when we drilled away were cancelled to free the workers, so
        # we only re-offer the completed ones.
        cached = sess.hotspot_cache.get(sess.depth)
        ready = {bid: b for bid, b in (cached or {}).items() if b.image_b64}
        if ready:
            sess.branches = ready
            for b in ready.values():
                b.status = "ready"  # a previously-chosen one is clickable again
            async for event in _stream_prefetch(sess, list(ready.values())):
                yield event
            return
...
async def click(session_id: str, hotspot_id: str) -> AsyncIterator[str]:
    ...
    try:
        ...
        # Promote the chosen branch. Free the workers for the NEXT layer by
        # cancelling siblings still generating; finished siblings keep their
        # cached image so Back can still serve them instantly.
        branch.status = "chosen"
        _free_workers(sess.branches, keep=branch)
        sess.archive.extend(sess.branches.values())
        sess.depth += 1
