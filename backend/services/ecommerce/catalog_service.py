...
class EcommerceCatalogService:
    ...
    async def upload_image(self, file: UploadFile) -> dict:
        ...
        cached = cache_get_json(f"ecommerce:image-hash:{image_hash}")
        if cached:
            cached_filename = cached.get("filename")
            if isinstance(cached_filename, str) and (settings.UPLOADS_DIR / cached_filename).exists():
                return {
                    "imageId": cached["imageId"],
                    "imageHash": image_hash,
                    "imageUrl": f"/uploads/{cached_filename}",
                    "fromCache": True,
                }
        ...
        cache_payload = {"imageId": image_id, "filename": image_filename}
        cache_set_json(f"ecommerce:image:{image_id}", cache_payload)
        cache_set_json(f"ecommerce:image-hash:{image_hash}", cache_payload)
        return {
            "imageId": image_id,
            "imageHash": image_hash,
    ...
    async def identify_item(self, image_id: str, x: float, y: float) -> dict:
        image_filename = self._lookup_uploaded_image(image_id)
    ...
    def _lookup_uploaded_image(self, image_id: str) -> str | None:
        cached = cache_get_json(f"ecommerce:image:{image_id}")
        cached_filename = cached.get("filename") if cached else None
        if isinstance(cached_filename, str) and (settings.UPLOADS_DIR / cached_filename).exists():
            return cached_filename

        for filename in os.listdir(settings.UPLOADS_DIR):
            if filename.startswith(image_id):
                cache_set_json(f"ecommerce:image:{image_id}", {"imageId": image_id, "filename": filename})
                return filename
        return None
