...
# 5. Run
npm run dev        # API :8001 + UI :5173

# Or run them separately:
npm run backend    # API :8001
...
| `DATABASE_URL` | Auth | Default local SQLite DB at `./data/drilldown.db`; use Postgres for shared/prod environments |
...
| `VITE_API_BASE` | Frontend | Optional API override; blank uses the Vite dev proxy |
...
| Script | Command |
|--------|---------|
| `npm run backend` | Start FastAPI on :8001 |
